 ...
 *
 * **What a timeout leaves behind** is store.md §14(58).  In one line:
 ...
 * anything is written.  §14(60) argues the widening.
 *
 ...
 * on the peer for as long as the peer takes (§14(59)).
 */
 ...
 * connection and has nothing to be told the end of (§14(59)).
 */
static void
ninefree(Nine *c)
{
 ...
 * one newfid (§14(60)).  Answers 1 with the offending fid in *busy.
...
ninegot(Nine *c, int n)
{
	...
	case Ngone:
		 ...
		 * where it arrives (§14(58)); the tag stays held until the
 ...
 * (§14(59)): a peer that hangs up is one to dial again, and a peer
 ...
 * transport death.  `msize' is passed in because the reader has it to
 * hand from the loop it came out of, where it read it under lk.
 *
 * It is settled once, in nineopen, and that store runs with both
 * procs already made, so what orders it is not a lock but where the
 * reader is: the Rversion took nexpect back to 0, so the reader is
 * parked in rsleep and reads nothing until a caller's nineput takes
 * lk and wakes it — and the first such caller is the one nineopen
 * answered the handle to, after the store.  The timer never looks at
 * msize, and every other read of it (nineread, ninewrite, nineput) is
 * a caller's, after the handle escaped.
 */
static int
 ...
 * back only when the peer speaks or `hangup' breaks the fds (§14(59)).
 */
static void
ninereader(void *a)
{
 ...
 * write(2) to find that out (§14(58)).  The mark this reads is the
 ...
 * peer is dead for this client's purposes (§14(58)).  It is recorded
	...
	Fcall *f, uchar **mp)
{
	...
	*mp = nil;
	...
	if(out == Ninetimeout){
		 ...
		 * Rflush (§14(58)), and so does the fid.
		 */
		q->state = Ngone;
		if(i > 0){
...
nineopen(Ninecfg *cfg)
{
	 ...
	 * size below the floor is reported and not refused (§14(57)).
	 ...
	 * told nothing about its release (§14(59)).
	 */
	qlock(&c->lk);
	c->freed = cfg->freed;
 ...
 * §14(60) holds ANY fid a request names, so both are busy for the
 ...
 * target one newfid at once (§14(60)).
 *
 ...
 * flushes its own tag and needs nothing here (§14(58)); this is for a
