...
`drop`, `forget`, `reclaim`, `reconcile`, `advert`, `fence off` — MUST
fail with
`fenced`, except that `fence off` MUST fail that way only while a
**lease-derived** fence (F1) is in force (D25). `reclaim` is in that
set because the walk it starts discards records (§1.5): a fenced
instance is one whose map may be stale, and a discard is decided
against that map's `tombdays` and epoch. Only `refresh`,
