...
| `scrub` | admin | `scrub [start\|stop] [rate=<n>]` | Control the scrubber; `rate` in KiB/s. §7.5 has each instance scrub on a schedule of its own, so `start` asks for a pass now rather than being the only thing that runs one, `stop` ends the pass that is running, and neither word turns that schedule off. | `bad ctl` |
