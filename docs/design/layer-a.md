...
| `scrub` | admin | `scrub [start\|stop] [rate=<n>]` | Control the scrubber; `rate` in KiB/s. §7.5 has each instance scrub on a schedule of its own, so `start` asks for a pass now rather than being the only thing that runs one, `stop` ends the pass that is running, and neither word turns that schedule off — `stop` does not hold the next tick back, and `start` does not move it. | `bad ctl` |
| `reclaim` | admin | `reclaim [start\|stop]` | Control the tombstone reclaim walk (§1.5): start a pass over this instance's tombstones now, or stop the one running. An instance that discards at all SHOULD run the walk on a schedule of its own — a period of `tombdays`/2 matches the `bump` cadence condition 3 waits for (§8.3) — so the verb asks for a pass early rather than being the only thing that runs one, and neither word turns that schedule off — `stop` does not hold the next tick back, and `start` does not move it. `start` alone is fenced. | `fenced` (`start`), `bad ctl` |
