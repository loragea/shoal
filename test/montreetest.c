 ...
 * asserts the rule it is about rather than a literal: what §14(56)
...
/*
 * A monitor that has never published.  store.md §14(49): the start is
...
/*
 * store.md §14(49)'s third clause: a current map that IS there and
...
tbadmap(void)
{
	 ...
	 * (store.md §14(54)).  An unformatted partition is the cheapest
	 * one to reach.
	 */
	if((d = simopen(Tsecsz, Tnsec, Tseed)) == nil)
		sysfatal("simopen: %r");
...
/* §8.1's attach grammar, and store.md §14(53)'s fills */
static void
tattach(void)
{
	static struct {
 ...
 * store.md §14(53)'s /map.next cell said the other way round.
 */
static void
ttree(void)
{
...
/*
 * §8.1's role matrix, every cell, exact strings.  store.md §14(53) is
...
tmatrix(void)
{
	static struct {
		 ...
		 * columns — follows it (store.md §14(53)).  The instance's
	 ...
	 * the reader may not write /map at all (store.md §14(54)).
	 */
...
tmaps(void)
{
	...
	/* leading zeros are part of the number (store.md §14(50)) */
	clwalk1(&cl, Froot, Ff, "maps", &r);
	...
	/* the two refusals of store.md §14(50) */
	clwalk1(&cl, Froot, Ff, "maps", &r);
 ...
 * path may do (store.md §14(50), §10).  Two ring entries then carry
...
/*
 * §8.4's evidence, as store.md §14(51) defines it: a Tread of /map on
...
/*
 * store.md §14(51)'s currency predicate.  /map is snapshot-at-open,
