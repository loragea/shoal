 ...
 * grains the process holds reserved (store.md §14(42)).  A reservation
...
tgrammar(void)
{
	...
	/*
	 * store.md §14(45): a header line longer than 1024 bytes carries
 ...
 * §14(42)).  What is NOT a divergence: an equal key whose csum already
 ...
 * per-handle charge this per-fid bound (store.md §14(42)), so what
 ...
 * marks it dead and leaves the handle alone (store.md §14(44)), and a
 * chunk naming a second object, which hashes to another queue and runs
 * beside it (§14(42)).  Neither may release the handle the busy chunk
 * is about to write through — the look that chunk takes when its call
 * returns is the one release there is (§14(43), §14(44)) — and a
 ...
 * `busy' whose `g' is still nil (obj.c).  §14(43) and §14(44) hold
 ...
 * §14(44), obj.c's srvstagelive and srvstagefinal).  A look that
 ...
 * inside a step on it, dead or not (store.md §14(44)).  Taking it
...
tchannel(void)
{
	 ...
	 * a second helping (store.md §14(46)).
	 */
	if(chanop(&cl, Frpc, "op=meta oid=alpha epoch=7", nil, 0, &r) != Rwrite)
	 ...
	 * free to carry the next exchange (store.md §14(41)).
	 */
	waitidle(ctx, &np0, &nd0);
	srvhook(ctx, "objhold", 1);
	memset(&t, 0, sizeof t);
...
tlist(void)
{
	 ...
	 * lines cannot say the inventory is over (store.md §14(46)).
	 */
